...
namespace NetBy.Business.Servicios
{
    public class ComprasService : IComprasService
    {
        ...
        public ComprasService(DataContext context, IProductosService productosService)
        {
        ...
        public List<Compras> GetAll(FilterBase filter, PaginationFilter pagination)
        {
            List<Compras> result = default;
            using (var transaction = CreateTrancationAsync())
            {
                ...
                {
                    result = _context.INV_Compras.Include(x => x.Detalles)
                                       .ToList();
                }
                else
                {
        ...
        public async Task<ActionModel> Create(Compras entidad)
        {
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    _context.INV_Compras.Add(entidad);
        ...
        public async Task<ActionModel> Update(Compras entidad)
        {
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ActionModel respuesta = new ActionModel();
                    var entidadToUpdate = _context.INV_Compras.Include(x => x.Detalles).FirstOrDefault(x => x.Id == entidad.Id);
                    if (entidadToUpdate == null)
                    {
                        respuesta.Success = false;
                        respuesta.Mensaje = "No se encontró la compra a editar.";
                        return respuesta;
                    }
                    ...
                    entidadToUpdate.Detalles = new List<ComprasDt>();
                    ...
                    if (entidad.Estado.Equals("PROCESADO"))
                    {
                        foreach (var item in entidad.Detalles)
                        {
                            ...
                            var stockActual = existeStock?.UnidadesStock ?? 0;

                            Stock stock = new Stock
                            {
                                ProductoId = item.ProductoId,
                                UnidadesStock = stockActual + item.Cantidad
                            };
                    ...
                    _context.INV_Compras.Update(entidadToUpdate);
                    _context.SaveChanges();
        ...
        public async Task<ActionModel> Desactivate(int compraId)
        {
            using (var transaction = await _context.Database.BeginTransactionAsync())
            {
                try
                {
                    ActionModel respuesta = new ActionModel();
                    var entidadToUpdate = _context.INV_Compras.Include(x => x.Detalles).FirstOrDefault(x => x.Id == compraId);
                    if (entidadToUpdate == null)
                    {
                        respuesta.Success = false;
                        respuesta.Mensaje = "No se encontró la compra a eliminar.";
                    ...
                    if (entidadToUpdate.Estado.Equals("PROCESADO"))
                    {
                        foreach (var item in entidadToUpdate.Detalles)
                        {
                            ...
                            if (stock.UnidadesStock - item.Cantidad < 0)
                            {
                                respuesta.Success = false;
                            ...
                            //reverso kardex
                            var kardex = _context.INV_Kardex.FirstOrDefault(x => x.TransaccionId == compraId && x.ProductoId == item.ProductoId);
                            if (kardex == null)
                            {
                                respuesta.Success = false;
